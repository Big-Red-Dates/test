...
function handleAjaxRequest(resultBean, status,XMLHttpRequest)
{
	if(!resultBean.success)
	{
		if(resultBean.messageCode=="01")
		{
			...
			window.top.location.href=getContextPath()+"/operation/";
			return false;
	...
	if(ajaxRequestChecking!=null)
	{
		if(ajaxRequestChecking=="01")
		{
			checkLoginError_addbackdrop();
			alert('您未登录或会话已过期');
			window.top.location.href=getContentPath()+"/operation/";
			return false;
...
function topLoading()
{
	...
	{
		$("body").append("<div id='topLoadingDiv' class='top_loading'><img border='0' src='"+getContextPath()+"/operation/resources/images/loading.gif'/></div>");
	}
